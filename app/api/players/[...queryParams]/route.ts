...
import { compProfileCollectionGroup, competitionProfilesSubcollection } from "config/server";
...
export async function GET(_request: Request, { params }: { params: { queryParams: Array<string | undefined> } }): Promise<NextResponse<PlayersResponseType>> {
	...
	try {
		...
		const querySnapshot = await compProfileCollectionGroup.orderBy("rating.displayRating").limit(Number(number) || 5).get();
		return NextResponse.json(querySnapshot.docs.map((doc) => ({ ...doc.data(), id: doc.id })));
	} catch (error: any) {
		console.log(error);
		throw Error(error);
